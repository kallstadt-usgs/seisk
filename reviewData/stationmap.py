...
def stationmap(event_id, chanshow='', maxradius=100):
    ...
    event_lat, event_lon, event_time, datsource = findsta.get_event_info(event_id)
    lats = np.array(lats)
    lons = np.array(lons)
...
def stationmap_specify(stationshow, event_id, chanshow='', maxradius=100):
    """
    Plot specified station names for an event in the lsseis database
    USAGE
    fig = stationmap_specify(stationshow, event_id, chanshow='', maxradius=100)
